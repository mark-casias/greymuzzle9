...
class ViewExecutable {
  ...
  protected function _buildArguments() {
    ...
    foreach ($this->argument as $id => $arg) {
      ...
      if (isset($arg) || $argument->hasDefaultArgument()) {
        ...
        // Test to see if we should use this argument's breadcrumb title
        if (!empty($argument->options['breadcrumb_enable']) && !empty($argument->options['breadcrumb'])) {
          $breadcrumb_title = $argument->options['breadcrumb'];
        }
      }
      else {
    ...
    if (!empty($breadcrumb_title)) {
      $this->build_info['breadcrumb_title'] = $breadcrumb_title;
    }
  ...
  /**
   * Gets the view's current breadcrumb title.
   *
   * This can change depending upon how it was built.
   *
   * @return string|false
  ...
  public function getBreadcrumbTitle() {
    ...
    // During building, we might find a breadcrumb override. If so, use it.
    if (!empty($this->build_info['breadcrumb_title'])) {
      $title = $this->build_info['breadcrumb_title'];
    }
    else {
      $title = $this->display_handler->getOption('title');
    }
